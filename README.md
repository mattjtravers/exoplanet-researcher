...
This project implements a Spec-Driven Development (SDD) workflow using [OpenSpec](https://github.com/Fission-AI/OpenSpec/) to improve system predictability, enforce strict interface boundaries, and minimize configuration drift.
...
* **Type-Driven Orchestration via `pydantic_graph`:** The application uses a native `pydantic_graph` state machine as an alternative to larger orchestration frameworks. Managing the control flow strictly through Python type hints eliminates external configuration abstractions, reducing runtime memory overhead and providing explicit visibility over state transitions.
